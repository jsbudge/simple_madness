...
results = pd.DataFrame(columns=[100, 128, 200, 255, 301, 512])
...
for degree in range(2, 7):
    ...
    for n_est in results.columns:
        ...
        for test, train in s_cv.split(X_nfeat, y_feat.index):
            ...
            res = []
            for _ in range(100):
                test_br = applyResultsToBracket(test_br, rfc_results, select_random=True, random_limit=.5)
...
final_results = pd.DataFrame(index=ps.index, columns=['Res', 'res1'], data=rfc.predict_proba(ps))

ps = getPossMatches(nfeats, 2026, True, False, datapath, 'W')

final_results = pd.concat([final_results, pd.DataFrame(index=ps.index, columns=['Res', 'res1'], data=rfc.predict_proba(ps))])
final_results.to_csv(f'{datapath}/rfc_results.csv')
