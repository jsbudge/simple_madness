...
class NeuralKF(LightningModule):
    ...
    def __init__(self, state_sz: int, force_sz: int, meas_size: int = 4, alpha: float = 1e-6, k: float = 0., beta_filter: float = 2.):
        super().__init__()
        self.save_hyperparameters()
        self.automatic_optimization = False

        self.q = nn.Parameter(torch.randn(state_sz), requires_grad=True)
        self.R = nn.Parameter(torch.randn(state_sz), requires_grad=True)
        self.p = torch.zeros((state_sz, state_sz))
        ...
        self.covar_weights = torch.tensor(np.array([(self._lambda / (state_sz + self._lambda)) + (1 - alpha ** 2 + beta_filter)
                                       ...
                                       range(self.n_sigmas)]), dtype=torch.float32)
        self.mean_weights = torch.tensor(np.array([(self._lambda / (state_sz + self._lambda))
                                      if i == 0 else 1 / (2 * (state_sz + self._lambda)) for i in range(self.n_sigmas)]), dtype=torch.float32)
    ...
    def get_sigmas(self, x):
        ...
        ret = torch.zeros((self.n_sigmas, self.hparams.state_sz), dtype=torch.float32, device=self.device)

        tmp_mat = (self.hparams.state_sz + self._lambda) * self.p

        # print spr_mat
        spr_mat = sqrtm(tmp_mat)

        ret[0] = x
        for i in range(self.hparams.state_sz):
            ret[i + 1] = x + spr_mat[i]
            ret[i + 1 + self.hparams.state_sz] = x - spr_mat[i]
    ...
    def predict(self, sigmas, dt):
        ...
        self.p = self.covar_weights * diff.T @ diff

        self.p += dt * torch.diag(self.q)
    ...
    def update(self, curr_x, sigmas, data):
        ...
        p_yy = self.covar_weights[:, None] * y_diff.T @ y_diff
        p_yy += torch.diag(self.R)

        p_xy = self.covar_weights * x_diff.dot(y_diff.T)
