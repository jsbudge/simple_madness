import pandas as pd
from bracket import generateBracket, applyResultsToBracket, calcKHighestProbBracket
...
for i in range(10):
    test_br = applyResultsToBracket(test_br, results_frame, select_random=True, random_limit=.5)
    with open(f'./brackets/{model}_{i}_({abs(test_br.log_likelihood):.3f}_mens).txt', 'w') as f:
        f.write(str(test_br))

check = calcKHighestProbBracket(test_br, results_frame, 5, 1000)
with open(f'./brackets/{model}_overall_(mens).txt', 'w') as f:
    f.write(str(check))
...
check = calcKHighestProbBracket(test_br, results_frame, 5, 1000)
with open(f'./brackets/{model}_overall_(womens).txt', 'w') as f:
    f.write(str(check))
